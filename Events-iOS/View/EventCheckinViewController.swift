...
class EventCheckinViewController: UIViewController, ValidationDelegate {
    ...
    func setup(eventId: String) {
        viewModel = EventCheckinViewModel(eventId: eventId,
                                          bindToViewController: onRequest,
                                          bindLoadingChange: onLoadingChange)
    }

    func setupValidator() {
        if let nameField = self.nameField {
            validator.registerField(nameField, errorLabel: nameErrorLabel,
                                    rules: [RequiredRule(message: "Campo obrigatório")])
        }

        if let emailField = self.emailField {
            validator.registerField(emailField, errorLabel: emailErrorLabel,
                                    rules: [RequiredRule(message: "Campo obrigatório"),
